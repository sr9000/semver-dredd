...
from cli.utils import (DEFAULT_BAKED_FILE, DEFAULT_CURRENT_FILE,
                       EXIT_BREAKING_CHANGES_DETECTED, EXIT_ERROR, EXIT_OK,
                       _generate_snapshot_yaml, _get_change_descriptions,
                       _get_language_plugin, _print_level,
                       _resolve_snapshot_class, _run_diff,
                       _severity_for_change, _should_use_color,
                       _warn_snapshot_plugin_mismatch)
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    # Warn if baked snapshot was generated by a different plugin.
    # Read provenance from raw YAML (plugins may not expose generator as a field).
    try:
        import yaml as _yaml

        _baked_raw = _yaml.safe_load(baked_path.read_text()) or {}
        _baked_gen = _baked_raw.get("generator") or {}
        _baked_plugin = str(_baked_gen.get("plugin_name", "")).lower()
        if _baked_plugin and _baked_plugin != plugin_name:
            _warn_snapshot_plugin_mismatch(
                _baked_plugin, plugin_name, use_color=use_color
            )
    except Exception:
        pass  # Provenance check is best-effort; never block status.
