...
from cli.utils import (EXIT_BREAKING_CHANGES_DETECTED, EXIT_OK,
...
def cmd_compare(args: argparse.Namespace) -> int:
    ...
    # Warn when old and new snapshots were generated by different plugins.
    old_gen = getattr(old_snapshot, "generator", None)
    new_gen = getattr(new_snapshot, "generator", None)
    if (
        old_gen is not None
        and new_gen is not None
        and old_gen.plugin_name
        and new_gen.plugin_name
        and old_gen.plugin_name.lower() != new_gen.plugin_name.lower()
    ):
        _warn_snapshot_plugin_mismatch(
            old_gen.plugin_name, new_gen.plugin_name, use_color=use_color
        )
