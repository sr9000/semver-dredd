...
master:          root ── c2 ── c3 ── … ── tip
                   \
bench/easy-v1:      └── baseline   (one commit: full tree + scaffolding + SEEDED defects)
                          \
bench/easy-v1-<model>:     └── model's own commits…
```

This also makes collection trivial: a model's entire contribution is
`git diff bench/easy-v1 bench/easy-v1-<model>`.
...
# … add reports/TASK.md …
git add -A
git commit -m "bench: easy-v1 baseline"
```
...
```bash
# on bench/easy-v1, after editing the files:
...
e.g. `~/bench-keys/easy-v1.md`. It must not be readable by candidates.
...
```bash
git branch bench/easy-v1-<model> bench/easy-v1
```
...
```bash
bash scripts/bench/stash-from-branch.sh bench/easy-v1 bench/easy-v1-<model> <model-name>
```
...
> 1. `git checkout bench/easy-v1 && git checkout -- . && git clean -fd`
