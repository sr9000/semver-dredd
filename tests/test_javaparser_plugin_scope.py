"""Use-case tests for JavaParser plugin package-prefix include/exclude scope.

Mirrors tests/test_java_plugin_scope.py using the same fixture package layout
(tests/fixtures/java_scope/) since both plugins share the documented
package-prefix scope semantics.
"""
...
from semver_dredd_javaparser.plugin import JavaParserPlugin, JavaParserSnapshot
...
def _snapshot_names(options: dict | None = None) -> set:
    plugin = JavaParserPlugin()
    result = plugin.generate_snapshot(str(FIXTURES_DIR), "1.0.0", options=options)
    assert result.success, result.error_message
    snap = JavaParserSnapshot.from_yaml_str(result.yaml_content)
